package dev.cyberjar.embabeldemo.migration;
...
public class SchemaDataInitializerChangeUnit {
    ...
    public void beforeExecution() {
        ...
        civilianOps.createIndex(new Index().on("nationalId", Sort.Direction.ASC).unique());

        IndexOperations implantOps = mongoTemplate.indexOps(Implant.class);
        implantOps.createIndex(new Index().on("serialNumber", Sort.Direction.ASC).unique());
        implantOps.createIndex(new Index().on("lotNumber", Sort.Direction.ASC));


        IndexOperations logOps = mongoTemplate.indexOps("implant_logs");

        logOps.createIndex(new Index().on("implantSerialNumber", Sort.Direction.ASC));
        logOps.createIndex(new Index().on("timestamp", Sort.Direction.DESC));
        logOps.createIndex(new GeospatialIndex("location").typed(GeoSpatialIndexType.GEO_2DSPHERE));

        logOps.createIndex(new Index().on("timestamp", Sort.Direction.ASC).expire(Duration.ofDays(90)));
